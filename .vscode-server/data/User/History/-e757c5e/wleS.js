import React, { useState} from "react";
import axios from "axios"
import './styles/App.css'
import Button from "./Button";
import PostItem from "./PostItem";
import PostList from "./PostList";
function App() {
const [post, setPost] = useState('hello');
const [list, setList] = useState([])
const [posts, setPosts] = useState([{header : 'js'}])
const [fecthing, setFething] = useState([{}])

const createPost = (newPos) => {
  setList([...list, newPos]);

const createPosts = (e, {id}) => {
  e.preventDefault();
  const newPos = {
    ...posts,
    id: Date.now()
    }
    createPost(newPos)
  setPosts({header : ''});
}

  const fetch = async(e) => {
    e.preventDefault()
    const query = await axios.get('http://jsonplaceholder.typicode.com/posts');
    
    console.log(query)
    setFething( query)
  }

 
  return (
    <div className="App">
    <form className="form">
      <h1>{post}</h1>
      <input type="text" value={post} onChange={(e)=>setPost(e.target.value)}/>
      <Button post={post} setPost={setPost}>Нажми на меня</Button>
      <div>{post.split(',').map((p) => <h3 key={p.id}>{p}</h3>)}</div>
      <PostItem props={post} number={1}>I iam Ilya</PostItem>
      <PostList prop={posts}>PostList1</PostList>
      <PostList prop={posts}>PostList2</PostList>
     <form>
      <input type="text" value={posts.header} onChange={(e) => setPosts({...posts, header: e.target.value})}></input>
      <button onClick={createPosts}>click</button>
      </form>
      <div>
        <button onClick={fetch}>FETCH</button>
      {[fecthing].map((q) => {
        if(q.id < 10) {
          return <h1 key={q.id}>{q.title}{q.body} hello</h1>
        }
        return "not post"
      }
      )} 
      </div>
      </form>
     </div>
  )
  }
};
export default App;
